...
function GetNearestEnemy(x, y)
    ...
    local minDistance = DistanceSquared(x, y, enemy.position.x, enemy.position.y)
    local nearestEnemy = enemy
    
    for i = numEnemies - 1, 1, -1 do
        enemy = enemyPool.activeList[i]
        local distanceSquared = DistanceSquared(x, y, enemy.position.x, enemy.position.y)
...
function GetNearestCargo(x, y)
    ...
    local minDistance = DistanceSquared(x, y, cargo.position.x, cargo.position.y)
    local nearestCargo = cargo
    
    for i = numCargo - 1, 1, -1 do
        cargo = cargoPool.activeList[i]
        local distanceSquared = DistanceSquared(x, y, cargo.position.x, cargo.position.y)
        if distanceSquared < minDistance then
            minDistance = distanceSquared
            nearestCargo = cargo
        end
...
function CheckEnemyCargoCollisions(e)
    ...
    for i = numCargo, 1, -1 do
        ...
        if CirclesOverlap(e.position.x, e.position.y, e.collisionRadius, cargo.position.x, cargo.position.y, cargo.collisionRadius) then
            e.attachedCargo = cargo
            e.attachX = cargo.position.x - e.position.x
            e.attachY = cargo.position.y - e.position.y
...
function UpdateEnemy(e)
    ...
    elseif e.state == ENEMY_STATE_STEAL then
        e.target = GetNearestCargo(e.position.x, e.position.y)
    elseif e.state == ENEMY_STATE_FLEE then
        e.target = {}
        e.target.position = {}
        local dirX = 0
        local dirY = 0
        dirX, dirY = VectorTo(player.position.x, player.position.y, e.position.x, e.position.y)
        e.target.position.x = e.position.x + dirX * 100
        e.target.position.y = e.position.y + dirY * 100
    end
    ...
    if e.state == ENEMY_STATE_FLEE and e.attachedCargo ~= nil then
        e.attachedCargo.position.x = e.position.x + e.attachX
        e.attachedCargo.position.y = e.position.y + e.attachY
        
        if e.attachedCargo.position.x > SCREEN_WIDTH or
            e.attachedCargo.position.y > SCREEN_HEIGHT or
            e.attachedCargo.position.x < 0 or
            e.attachedCargo.position.y < 0 then
...
function UpdateProjectile(e)
    if e.isHoming == true then
        e.target = GetNearestEnemy(e.position.x, e.position.y)
        UpdateWobbleSeek(e)
    else
        UpdateEntity(e)
    end
...
function UpdateProjectiles(playerWantsFire)
    ...
    if player.fireTimer <= 0.0 and playerWantsFire and projectilePool.hasFree() then
        ...
        projectileParams.x = player.position.x
        projectileParams.y = player.position.y
...
function CheckProjectileEnemyCollisions()
    ...
    for i = numProjectiles, 1, -1 do
        ...
        for j = numEnemies, 1, -1 do
            ...
            if CirclesOverlap(projectile.position.x, projectile.position.y, projectile.collisionRadius, enemy.position.x, enemy.position.y, enemy.collisionRadius) then
                ...
                explosionParams.x = projectile.position.x
                explosionParams.y = projectile.position.y
...
function CheckEnemyPlayerCollisions()
    ...
    for i = numEnemies, 1, -1 do
        ...
        if CirclesOverlap(player.position.x, player.position.y, player.collisionRadius, enemy.position.x, enemy.position.y, enemy.collisionRadius) then
...
function LockPlayerToScreen()
    if player.position.x < player.width / 2 then player.position.x = player.width / 2 end
    if player.position.y < player.height / 2 then player.position.y = player.height / 2 end
    if player.position.x > SCREEN_WIDTH - player.width / 2 then player.position.x = SCREEN_WIDTH - player.width / 2 end
    if player.position.y > SCREEN_HEIGHT - player.height / 2 then player.position.y = SCREEN_HEIGHT - player.height / 2 end
end
...
function GameState_Start()
    if not gameInProgress then
        defeatSfx = LoadSound("spaceGame/sfx/explosion.wav")
        hitSfx = LoadSound("spaceGame/sfx/hit.wav")
        spaceImage = LoadImage("spaceGame/space.png")
        
        InitPlayer()
        InitPools()
        InitCargo()    
            
        gameState.font = LoadFont("spaceGame/8_bit_pusab.ttf", gameState.fontSize)
        gameInProgress = true
        spawnTimer = 4000.0
        spawnDelay = 400
        ...
        gameState.playTime = 0
        gameState.tutorialTimer = 4000
                
        local pick = math.random(1, 3)
        
        if pick == 1 then
            gameState.music = LoadMusic("spaceGame/music/level1.ogg")
        elseif pick == 2 then
            gameState.music = LoadMusic("spaceGame/music/level2.ogg")
        elseif pick == 3 then
            gameState.music = LoadMusic("spaceGame/music/level3.ogg")
        end
                        
        PlayMusic(gameState.music)
        player.shootSfx = LoadSound("spaceGame/sfx/shoot.wav")
    end
...
function GameState_Draw()
    ...
    if gameState.tutorialTimer > 1500 then
